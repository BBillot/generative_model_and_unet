function [dist, ordInf, ordSup, absInf, absSup] = ...
    drawCells2(AxonsPatch, radius, SelectionPixel, brightness, height, width, sigma_noise_circle)
...
% the circle
dist=zeros(ordSup-ordInf+1,absSup-absInf+1);
for i=1:NSources
    dist(refAbs(i),refOrd(i)) = 0.5;
end
%dist(dist>radius) = Inf;

%convolve the obtained matrix with gaussian mask, to deform the image
MaskSize = ceil(4*radius+1);
g2 = fspecial('gaussian',MaskSize,radius/4);
...
dist(SelectionMatrix==SelectionPixel) = radius;

%adds white noise to the circle
dist = dist+sigma_noise_circle*randn(size(dist,1),size(dist,2));
