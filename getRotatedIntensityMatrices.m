function [rotatedCopies,rotatedSegmentations,rotatedAxonsGTPointsWithoutGap,rotatedAxonsGTPointsWithGap,width,height] = getRotatedIntensityMatrices...
...
for i=2:NbImages
    ...
    rotatedAxonsGTPointsWithoutGap(:,:,i) = [tempGTPWithoutGap(1,:)+width/2;tempGTPWithoutGap(2,:)+height/2];
    ...
    rotatedAxonsGTPointsWithGap(:,:,i) = [tempGTPWithGap(1,:)+width/2;tempGTPWithGap(2,:)+height/2]; %rotated coordinates in the original system
    
     % get corresponding segmentation image without gaps
    [tempAxonsDistWithoutGap,~,~,~,~,~,~] = PixDistanceToAxon(width,height,rotatedAxonsGTPointsWithoutGap(:,:,i),3,0,0,variationsWithoutGap); %get distance matrix
    rotatedSegmentations(:,:,i) = (tempAxonsDistWithoutGap <SegmentationThreshold);
    
    % get corresponding clean image with gaps
    [tempRotatedPatch,~,~,rotatedPatchVar,~,~,~] = PixDistanceToAxon(width,height,rotatedAxonsGTPointsWithGap(:,:,i),3,0,0,variationsWithGap); %get distance matrix
