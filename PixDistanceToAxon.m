function [BranchDistWithoutGap,BranchDistWithGap,BranchVariationsWithoutGap,BranchVariationsWithGap,GapSize,...
    newAxonGTPoints,newVariations]= PixDistanceToAxon(height,width,AxonGTPointsWithoutGap,thickness,MinGapSize,MaxGapSize,variations)
...
BranchDistWithoutGap = Inf*ones(height,width);            % Create a distance matrix initialised with "inf"
BranchVariationsWithoutGap = Inf(height,width);           % Create a variation matrix initialised with "Inf"
...
for n = 1:size(newAxonGTPoints,2)
    if newAxonGTPoints(1,n)>0 && newAxonGTPoints(1,n)<=height && newAxonGTPoints(2,n)>0 && newAxonGTPoints(2,n)<=width
...
if Case
    %gap part
    for n=indicesGapPoints(1):indicesGapPoints(end)
